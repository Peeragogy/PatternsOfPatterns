...
need not be run in a fixed order, but are interwoven together. Figure [[fig:placard]] provides a mnemonic.[fn:: For French speakers, /placard/ means ‘cupboard’, and there is another idiom, /mise en placard/, which applies to an employee whose tasks have all been reassigned.  While that is not quite the case here, it is potentially suggestive as another mnemonic.]
...
\end{echo}
For our present purposes, the main thing to notice is that using the three methods
...
PAR, reproduced in Appendix \ref{exemplar-par}).  Then, every six weeks
...
To make the way process works somewhat more clear, we include the data supporting the CLA from
\begin{echo}
PARs carried out in our 1\textsuperscript{st}, 10\textsuperscript{th}, and 18\textsuperscript{th} sessions.
The PARs are provided in full in Appendix \ref{appendix-pars}.\footnote{We did two PARs during the eighteenth session.}
\end{echo}
...
*** Supporting data from ERG’s first session
:PROPERTIES:
:UNNUMBERED: t
:END:
...
*** Supporting data from ERG’s tenth session
:PROPERTIES:
:UNNUMBERED: t
:END:
- /We’ve brainstormed a couple of options for getting out there: White-papers, Grants, Journal papers (very concrete)/
*** Supporting data from ERG’s eighteenth session
:PROPERTIES:
:UNNUMBERED: t
:END:
- /Alex: My major intention was to meet you guys and learn something, wanting to reinforce existing knowledge of emacs and develop it further/
*** Next Steps
:PROPERTIES:
:UNNUMBERED: t
:END:
...
but are, rather, descriptions of anticipated patterns of
...
by bidirectional links between patterns and next steps.
...
we may then look for ways to automate that.  In order to do the actual
...
typically are automated through new Lisp code, or with interfaces to
...
Broadly, a solution can often be decomposed into interrelated
subtasks.  If we are able to understand the (sub-)context and
(sub-)problems in detail, we can then make predictions about the way
an intervention could change the overall system.  As we will revisit
again later on in the paper, “two different forms of information
processing (bottom-up and top-down)” cite:Teufel2020.  CLA can help
with both.

Nominally, CLA is an analytic tool that decomposes a problematc
situation into layers and causes operating at and between these
...
observations into an overall plan.
...
Peeragogy patterns as a way to check our work and connect with a
broader community.  In this section, we will broaden our gaze
...
Form/ (/NSF/) recopied below as Figure \ref{synthesis-diagrams}.  Parts
...
** PAR: Emacs Research Group, February 13, 2021
<<exemplar-par-2>>
*** 1. Review the intention: what do we expect to learn or make together?
...
** PAR: Emacs Research Group, April 17, 2021: Special guest session
<<exemplar-par-3>>
*** 1. Review the intention: what do we expect to learn or make together?
...
** PAR: Emacs Research Group, April 17, 2021: Full session
<<exemplar-par-4>>
*** 1. Review the intention: what do we expect to learn or make together?
