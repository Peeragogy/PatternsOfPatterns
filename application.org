...
\citep[p.~247]{alexander1979a}.  Richard Gabriel reminds readers who
treat that sentence as a definition that Alexander then went on to
say:[fn:: https://wiki.c2.com/?PatternDefinition]
...
observe this kind of pattern in the way sand forms underwater ridges
subject to waves and tides, or the way a cell ‘solves’ a metabolic
problem.  The second sense is more designerly in nature.  Leitner
supplied a gloss that nicely encapsulates the way patterns, in this
sense, are meant to work: ``Patterns are shared as complete methodic
descriptions intended for practical use by experts and non-experts''
cite:leitner2015a.

In this paper, we ask: what happens if we think about patterns in the
second sense from the vantage point of the first sense?  There are
already several practical texts that use patterns$_2$ to talk about
patterns$_2$ — sharing methods that aid in discovery, writing,
workshopping, and application of pattern methods.  Our aim is to
develop a more fundamental analysis of patterns$_2$ as a phenomenon,
along the lines suggested by patterns$_1$.

Granted, the relevant forces are no longer simply physical, but are
socially distributed and often culturally determined.  This
underscores why such an analysis is both practical, and timely.  While
different communities think about patterns in different ways — e.g.,
architects, computer scientists, and institutional designers have had
quite different things to say about design patterns — all of these
communities face a now-or-never moment relative to climate crisis and
other massive upheavals.  They need to learn to work together.

In 1999, Alexander said as much whan he talked to computer scientists
about “the generation of a living world” cite:alexander1999a.  From
the vantage point of 2021, we cannot afford to take these remarks as
mere metaphor.  Neither can we afford blythe techno-optimism; nor to
confine our thinking about the matter to one domain, be it built
environment, software, or philosophy.  We hope that you’ll join us for
the journey.

\begin{echo}
\begin{table}
\begin{tabular}{llll}
\emph{People do:}           & perceive       & think            & act\\[.2cm]
\emph{Scientists call this:}& “sensory” & “cognitive” & “motor”\\[.2cm]
...
We detail and apply three techniques which can help a community carry
out self-evaluation and peer learning.  Each of the methods fills a
fundamental role (Table \ref{tab:acronyms}), and has natural use-cases
when considered in isolation.  When combined, they provide a more
holistic mixed pattern.  We mean this in a precise sense, as will be
described in due course.  Here’s how the methods work in brief:
...
transdisciplinary research seminar that studies the extensible, customizable, free/libre text editor, Emacs.[fn:: https://www.gnu.org/software/emacs/]
The role of this section is primarily illustrative: we show how we have
conjointly used PARs, CLA, and DPLs to develop a sense of direction for our
research, in concert with related efforts.  This case study provides a
...
the direction of travel of pattern language methods, helping us
...
what pattern languages can do in the future.
...
#+BEGIN_EXPORT latex
Appendices and notes provide supplementary material.
Several patterns in Appendix
\ref{appendix} are referred to by name the text.  Inline \textsc{Small
Caps} henceforth denotes references to external patterns, whereas ALL-CAPS
indicates patterns listed in this Appendix.  Footnotes are indicated with
numerical markers, while more extensive endnotes appear with alphabetic markers.\endnote{For example.}
#+END_EXPORT
...
\begin{echo}
\begin{flushright}
\emph{tl;dr: We detail the three methods we use in the paper.}
\end{flushright}
\end{echo}
...
shares common ground with the \textsc{Daily Scrum} and \textsc{Sprint
Retrospective} cite:sutherland2019a patterns from Scrum.  However,
...
/litany/, /system/, /worldview/ and /myth/.  This work draws on his scholarship of P. R. Sarkar.  Here, we explain each of the
...
The stress mentioned here is an upshot of
all of the activity described.  Business is booming, money is flowing: that’s not the problem.  Confusion has ensued
...
Like an ellipse, the concept of the design pattern has two main foci:
...
\citep[p.~311]{alexander1970a}.\footnote{Notwithstanding, Thich Nhat Hahn has written a book called \emph{\href{https://www.penguin.co.uk/books/111/1111997/how-to-sit/9781846045141.html}{How To Sit}} (2014), which somewhat proves Alexander’s point as the exception to the rule.}
\end{echo}
...
\begin{echo}
\begin{flushright}
\emph{tl;dr: We describe how we have jointly applied the three methods, PAR, CLA, and DPL.}
\end{flushright}
\end{echo}
...
\begin{echo}
We elaborated those bullet points into a narrative form, which we revised to accommodate new data as time went on. We also began to develop TODO items that would make the next steps for this seminar group both
actionable and meaningful.\endnote{ The Peeragogy approach to patterns is aligned with the feminist principle that all knowledge is incomplete (\url{https://mitpress.podbean.com/e/experiments-in-open-peer-review/}, minute 5). A “living” pattern is, accordingly, attached to Next Steps that would help to realise that pattern within a context; when we don’t have any next steps, we put the pattern in a \textsc{Scrapbook}.}
\end{echo}

8 See http://peeragogy.org/top for a reworking of the Peeragogy Handbook as a unified pattern languag
...
\begin{echo}
\begin{flushright}
\emph{tl;dr: We study DPL using CLA to understand the trends and themes.}
\end{flushright}
\end{echo}
...
\end{flushright}
\end{echo}
In 1999, the architect Christopher Alexander discussed
the future of design patterns with an audience of programmers.  We
have revisited this topic, centred on an application of a technique
from the field of future studies to the domain of patterns.  The
...
Table \ref{exemplar-par} is the first PAR prepared by the Emacs Research Group, whose work we will look at in more detail in our Case Study.
