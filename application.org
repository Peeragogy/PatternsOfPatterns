...
We’re not the only people to think about this but what is distinctive
about this paper is that we’ve done this with design pattern
terminology.
...
world.  You might get a song that matches the description but doesn’t
...
example, it might take the form of a suitable music theory (Chapter 1
Westergaard), one where you can say things like /ballabile/ (to indicate
...
and potential misfits.\footnote{Also: why only three rows?  According to G\"odel, any language which counts can serve as its own metalanguage.}

The second diagram refers to the process of design once we have arrived at the "formal
...
panel represents the complementary synthetic process in which one
...
patterns for use in the synthesis; and ultimately, 15 principles that could guide a design at an even more abstract level.

In this paper, we considered the problems of a group of people
organizing their activities.  The naive "actual world" approach (A.)
would be when a group takes a "seat of the pants" approach to dealing
with issues as they come up in the course of work.  PAR is a technique
for sketching a "mental picture"; CLA and Peeragogy patterns are,
respectively, techniques for analysis and synthesis at the "formal
picture" level.  Just as even a talented musician without a solid
grasp of music theory would be hard pressed to compose an augmentation
canon or symphony, so too we suggest that a group which faces complex
challenges may want to consider these as techniques for orchestrating
its activities.
...
\begin{equation*}
  \xymatrix{
    \hbox{\textbf{context}} & \hbox{\textbf{form}} & \hbox{\phantom{mental picture}} \\
...
\end{equation*}
\vskip 10pt
\emph{\textbf{C.}~Now we add a meta-language.}
\vskip 10pt
\begin{center}
...
\vskip 10pt
\end{center}
\caption{Diagrams from \emph{Synthesis of Form}}
\end{figure}
#+END_EXPORT
* Unnecessary :noexport:
