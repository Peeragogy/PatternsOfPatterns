...
cite:alexander1999a.  We believe this is more than a metaphor.
...
Software tools aid will managing this information, though our current
prototypes could certainly be improved.  These prototypes are based on
on Emacs’s built-in Org Mode (a tool for managing information
structured in outlines), Org Roam[fn:: [[https://www.orgroam.com/]]] (an
Emacs package layered on Org Mode and used for creating zettelkasten,
which are, essentially, wikis), and Org Roam Server (a further layer
for visualisation of the resulting network structures).  As described
in Section [[erg_worldview]], these tools have some deficiencies when
it comes to interlinking varied contents—such as patterns and next
steps—from distributed sources.  In short, we need to keep improving our tools in order to effectively manage growing distributed, interlinked, collections of PARs, CLAs, and DPLs, alongside other scientific and technical corpora.
...
Some key excerpts appear in Table [[tabone]].  At the same time, it is
worth noting that this is by no means the a complete critique.  As an
an example of one direction that we will not have time and space to
develop here, some applications of the concept of ‘gift culture’ have been
criticised as hegemonic in nature cite:Mallard2019: should we expect
pattern-theoretic, mob, or free/libre/open source software culture to
be immune from such critiques?  VanDrunen’s critique is useful for our
purposes not because it is the last word, but because it points to the
importance of considering the deeper layers in developing a concept.
There may also be a conflicts at these layers.
...
In an overview of their work on the Cooling the Commons pattern
language=, Cameron Tonkinwise and Abby Mellick Lopes write:
#+begin_quote
...
strategy might be study how practices changed in the past, as with the
...
extracted descriptions be used directly in computations?  One strategy
would be to think of design patterns as conceptual blends
cite:Corneli2018, which have a computational interpretation
...
this blends a learning space with cool refuge, and must balance these
...
relationships.  [[PLACARD][PLACARD]] methods could flips the Kostakis et
...
abilities—and methods—to perceive and work with patterns.
