...
treat that sentence as a definition that Alexander went on to
...
problem.  Design comes into play in the second sense.  Leitner
summarised how they are meant to work: ``Patterns are shared as
complete methodic descriptions intended for practical use by experts
and non-experts'' cite:leitner2015a.
...
already several practical texts that use patterns to talk about
patterns (always in the second sense): they share methods that aid in
discovery, writing, workshopping, and the broader application of
pattern methods.  Our aim is to develop a more fundamental analysis,
and to use this to work towards a new level of practicality in working
...
scientists, and institutional designers have had very different things
to say about design patterns.  Here, we combine patterns with methods
from future studies and elite training, aiming to show how these
methods can be used together to tackle the world’s biggest problems.

In 1999, Alexander spoke to computer scientists about “the generation
of a living world” cite:alexander1999a.  We cannot afford to take
these remarks as mere metaphor.  Towards this end, we need an
excavation and reworking of the design pattern theory.
...
more holistic mixed pattern.  We mean this in a precise sense, as will
be described in due course.  Here’s how the methods work in brief:
...
actionable knowledge surrounding a topic of concern.  Without doing
this, a group runs the risk of getting lost in the details.  CLA can
pull together information logged in PARs into a coherent body of
self-knowledge.  It can also help to surface concerns that might
remain implicit in pattern language: for example, CLA could help us
...
Having carefully analyzed the situation and identified a possible
solution, we must plot a course of action that accounts for the
...
\begin{echo}
Appendices and notes provide supplementary material.  Several patterns in Appendix \ref{appendix} are referred
  to by name the text.  Inline \textsc{Small Caps} henceforth denotes
  references to external patterns, whereas ALL-CAPS indicates patterns
  listed in this Appendix.  Footnotes are indicated with numerical
  markers, while more extensive endnotes appear with alphabetic
  markers.\endnote{\textbf{To PLoP 2021 Writers’ Workshop attendees:} In particular, we use end notes to stash potential outtakes.
    If you see something here that’s valuable, please let us know, and we may add it back into the paper.  If the material here
  doesn’t help, it may go out in the final submission.  We probably will not include endnotes in the final submission!}
\end{echo}
...
concerning.
\begin{echo}
The reader may think: “admittedly, I can see how the methods that have been discussed could help in understanding
the context and the problem, but is there anything here that can help with formulating solutions?”\endnote{\textbf{JC@coauthors:} I’m still a bit concerned about this!}
\end{echo}
This is a worthy concern, and something we will come back to later.
For now, the following section provides an integrated example of the [[PLACARD][PLACARD]] pattern in use.
...
\begin{echo}
Here, a range of media issues media begin to crop
...
the tensions and dynamics that exist at this level.
\begin{echo}
Alexander, for his part, tags Nietzsche as a nihilist, and distinguishes his own work
as comparatively hopeful \cite{alexander1991perspectives}.\endnote{For further
...
** Summary
A key benefit of this deep dive into DPL via CLA is that we have
...
We also have an answer to Alexander’s question, however, we’re not
sure if he’ll like it.  The ‘Chartres of programming’ has been hidden
...
We close with an epilogue sourced from Virgina Woolf.  For her, any
shock — or, we might say a crisis — was ‘followed by the desire to
explain it.’

\begin{quote}
\emph{It is only by putting it into words that I make it whole; this wholeness
...
[[second-exemplar-par]] is a PAR we developed after submitting the first
version of this paper to PLoP.
...
** PAR: Coauthors of Patterns of Patterns, July, 2021
<<second-exemplar-par>>
