...
provide a more holistic mixed pattern.  Our
...
comes up against complex global crises.
...
mindset” (/ibid./), embodied in a growing repository of technical fixes.
...
threatens life on Earth” (/ibid/.).  It is useful
...
As an
acronym, “PAR” has stood for various things over the years—Paragogical Action
Review,[fn:: http://ceur-ws.org/Vol-739/paper_5.pdf (p. 5) and /Peeragogy Handbook/ v3, p. 134]
Peeragogical Action Review,[fn:: https://github.com/Peeragogy/Peeragogy.github.io/wiki/Monthly-Wrap:-March-2020]
Project Action Review—but we like PAR as
...
tournament (the sum of the pars of each round).”—Wikipedia][fn:: https://web.archive.org/web/20150909224638/http://metameso.org/~joe/docs/The-Paragogical-Action-Review.pdf] Like the
...
In précis, in this study, we were interested in the following topics:
...
- The methods, workflows, and tools we are developing help us carry out our own activities thoroughly and efficiently, and could be of interest outside the group. How might similar processes be useful to others who interact /with/ Emacs?
...
- We want to encapsulate what we learn in new actionable and deliverable designs for software. What experiments can we do /on/ Emacs itself that might improve it?
...
However, the two studies together have given rise to an informal description of scenarios (presented in the following section)
which could be further broken down to frame subsequent work, using the Case Study
as a model.
...
communities/networks and histories to which they are heirs.  Taken together the
two studies offer a nuanced perspective on contemporary computer programming culture.

Thus, the Emacs Research Group is a small subcommunity of the larger Emacs
community, and inherits two core tenets at the /worldview/ level:
...
primarily linked with OO traditions.[fn:: https://wiki.c2.com/?DesignPatterns]
The members of these communities were not always in direct contact.
...
progress measurable?’.  We foresee the PAR→CLA→DPL workflow to be iterative, assisted by bidirectional
...
studies, we note that Schwartz cite:schwartz1996a (Appendix,
pp. 241-248, /viz./, his “Steps to Developing Scenarios”) described a process that
follows an outline that is strikingly similar to a design pattern
...
CLA.  We emphasize that these scenarios are not mutually exclusive.
...
points above, brings to mind Hesse’s /The Glass Bead Game/.  In his novel, Hesse
describes a society in which the community of scholars studying
abstract patterns forms a strong hierarchy, which, however, is out of touch
with practical realities experienced by outsiders.  This suggests to proceed
with caution.  When reflecting on futures-oriented discourses,
...
to describe soft skills that are useful for aspiring programming professionals cite:hoover2009apprenticeship,
however this falls far short of reforming the sector.  In order to reform the sector,
we would need to study the forces that make it how it is today.
...
and other review tools).  [[PLACARD][PLACARD]] as a whole is somewhat similar
to a method called Causal Layered Synthesis developed by Paul Wildman
...
analysis of DPL via CLA.  The patterns collected in /Fearless Change/ 
cite:manns2015a may be useful for negotiating uptake and development of the scenarios
outlined above.
...
\end{equation*}
\vskip 10pt
\emph{\textbf{B.}~Here, we add a mental picture that abstracts from the context (e.g., design requirements) and the form (e.g., plans).  In short, now work takes place mediated by a design process.  This allows specialization of labor, but there is no longer a direct unmediated link between $C1$ and $F1$.}
\vskip -20pt
\begin{equation*}
  \xymatrix{
    \hbox{\phantom{form}} & \hbox{\phantom{context}} & \hbox{\phantom{mental picture}} \\
...
“form” (the latter being analogous to what we have been calling
...
Our Case Study suggests ways in which progress could be
made towards realizing these scenarios.  Progress will become measurable through markers of
...
help people work with these methods.  As Alan Turing said regarding the
future of computing machinery and intelligence, we can see plenty that
needs to be done.

# CD: In final paragraph can you connect back to the Nikāya quote on page 2?
# JC+RP: Let’s leave this is an invitation to the reader to think this through for themselves... could be a whole be a whole other paper to confirm our general hypothesis related to the history of ideas.
...
cathedral” \cite{jodorowsky2009way} (p. 10).  Are pattern languages
the same sort of thing?  Consider that cathedrals are not just a
place to pray or to do private inward contemplation; they also have
meaningful overall designs, and are accompanied by beautiful images in the form of stained glass
windows to support people in understanding bible stories and otherwise
create an appropriate environment to inspire devotion.  Similarly,
design patterns typically come with images, and through venues like
PLoP, are associated with spaces of congregation.  With [[PLACARD][PLACARD]], we
have discovered a nexus that could bring wider communities together.
...
The image appearing in Figure 1 was made available under the terms of CC Zero by Lapsed Pacifist at =https://commons.wikimedia.org/wiki/File:Shell_to_Sea_placard.jpg=.
...
(Five years later, with the creation of Git, a certain form of forking became more
...
** Related work
- We could write something in Emacs to play the /Fearless Change/ game.
- Ray plans to implement a version of this that we could play together, you make paths and there are obstacles, and you try to see whether you can use the Fearless Change patterns to overcome the obstacles that people have made.
...
#+begin_quote
*Prompt:* "Here we will introduce you to each of your roles, their problems and contexts, to familiarize you with these things ..."
#+end_quote
...
introduction*.  It can include a one-page overview of how we expect
things to go (with a quick example and /an overview of the desired
outcome, clarifying our intention and stating where we see things to
be open to interpretation by participants/).

#+begin_quote
*Hypothesis:* We think that this method will allow people to come together to develop novel and meaningful solutions.
#+end_quote

- PLACARD is a /tool/ to bring people together people with different perspectives...
- ... so that they will come up with novel solutions to a problem
  - It will allow them to think more deeply about the reality than other methods
  - People will come from their own different existing backgrounds, and should adjust the roles to what they are actually already good at.

- All participants will participate in all aspects of PLACARD in the game phase
  - ... some will lead on various aspects
  - There will be some peer learning happening during the workshop
  - Part of the issue here is to understand what the problems are.

*** Overview
The four roles are linked with the four resources mentioned above:
...
/litany/, /system/, /worldview/, and /myth/.  I.e., the participants will be
asked to describe their problems, social context, worldview, and
narratives via a quick Q&A process.  (The Analyst will go a bit deeper
into CLA.)
...
This continues in breakout rooms per role.

*** Briefing for the *Historian* (intro to Neo-Carbon Energy scenarios)
#+begin_quote
*Prompt:* Your responsibility will be to know about the game world, and to say what has happened and something about what’s possible in this world. E.g., in the "Radical Startups" scenario it will be possible to *X and Y but not Z*.
#+end_quote

- How does the scenario holder relate to everyone else...?  They provide the context in which everything else unfolds.
- Give them the scenario, telling it as a story from the previous workshop as if it was history.
- They can set up the context and outlines the possible solutions as a story (working from the myth layer).
- They can reality-check the world that people are living in... a bit like a Game Master in a role-playing set-up

*** Briefing for the *Designer* (intro to Cooling the Commons cards and pattern methods)
#+begin_quote
*Prompt:* ...
#+end_quote

- Maybe closer to the /action/ function of DPL, i.e., coming up with a plan to implement the ideas that the group discussed.

*** Briefing for the *Kaijū Communicator* (intro to Flaws of the Smart City)
#+begin_quote
*Prompt:* Your responsibility will be to run the game in the next phase of the workshop.
#+end_quote

- Maybe closer to the /perception/ function of the PAR.
...
*** Briefing for the *Analyst* (intro to CLA)
#+begin_quote
*Prompt:* Your responsibility will be to guide people towards the outcome of your breakout group, to be reported back using the CLA format.  (Similar to Wrapper in peeragogy terms.)
#+end_quote
...
*** Overall training (15 minutes)

*** Small group on-site training (45 minutes)
...
\begin{center}
\begin{tabular}{llll}
Litany    & \textbf{Shopping Street} & \textsc{Street Parties \& Fêtes}  \\ % Say more about them here...
...
\noindent This might be narrated from inside the game world, roughly as follows:
