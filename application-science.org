...
comes up a climate crisis.  How are we to understand design patterns in this context?  Stephen Batchelor writes:
#+begin_quote
...
mindset” (/ibid./, here and in quotes throughout this paragraph), a growing repository of technical fixes.
...
# scales.
We may also reflect that while science fiction can provide a `thinking
machine' cite:Doherty2020, it cannot by itself provide historically
robust alternatives to the global crises that we face.  Again, we need
a concrete, testable, and adaptable approach that integrates thinking with action and
...
Research within a transdisciplinary research-and-development
seminar.[fn:: Not to be confused with the acronym PAR used in this
paper.]
...
community is not assumed to homogeneous, and, indeed, this can be part of how
they experience the conflict; i.e., it is not necessarily the case
that all members of the community share the same experience or view of
the context, or that they are all uniformly affected by
the circumstances arising.  The conflict is also referred to as a /problem/; its resolution is
described as a /solution/.  Alexander and Poyner emphasised that
...
Early on, Alexander described the need for patterns when things get complex
...
pattern, in Leitner’s sense of a complete methodic description cite:leitner2015a.  We
...
In précis, in Analysis we examined:
...
Alexander expands on the latter themes in his multi-volume work, /The Nature of Order/ (/TNO/). Our Analysis shows how
some of these more ephemeral-seeming factors are ramified across various layers of organisation.
...
breakdown of our Analysis.  We present directions for development in
the following sections, and, as such, the Analysis has given rise to
an informal set of high-level patterns.  Certainly, links to explicit
design patterns could be added to our narrative, using the Case Study
as a model, and this could make the vision we put forward more
actionable, and make progress more explicitly measurable.
...
computing, which is organised with various loose hierarchical
...
functional programming methods, applications of design patterns in computing are
typically more closely associated with the Object Oriented (OO) programming
style.  Indeed, the members of these communities were not
always in direct contact.  The =c2= pattern wiki was primarily linked
with OO traditions.
...
the availability of source code to encompass the right to reuse it.  A third
...
- /the sky/ and /the world/ (Alexander of /A Pattern Language/ (/APL/), as seen from the dual vantage points of a hearth and a gateway cite:benvenuto1993hermes).
...
relationship to other linked concepts like freedom, rights, ethics,
carbon emissions, migration, and broader systems design
considerations.  From this basis we can more thoroughly explore
dialectic tensions, and potentially initiate dialogue between
disparate communities.

Meanwhile, from our Case Study we can see, in practical terms, how
content like our CLA of design patterns could be developed further.  The
Case Study applied the [[PLACARD][PLACARD]] method within the Emacs Research Group
(ERG), which held regular meetings following EmacsConf 2020.[fn::
...
trajectory for our research on Emacs.  As a broader point of
...
These TODO items typically are not concrete objectives, but are,
rather, descriptions of anticipated patterns of behaviour—here linked
...
links between patterns and next steps, and assisted by software tools.
...
used to integrate these observations into an overall plan.
...
patterns cite:ostrom2009a (p. 11). Recent work looks at how description of such institutions can extracted from text
cite:Rice2021.  Could this line of thinking be extended, so that other similar kinds of patterns could be recognised where they appear?
...
move to open licensing—and suitable infrastructures for working with open contents?   And
what of the other kinds and qualities of openness that we surfaced in Section [[Results]]?
Simply using a ‘copyleft’ license is far from being a panacea cite:Krowne_Puzio_2006.
...
across different skill sets is challenging; and large scale
problems like adapting to climate
change can’t be solved if we carry on only doing business as usual.  In the Minnesota
...
have drawn upon, and perhaps through ensuing trial-and-error uptake or
adaptation of the methods we’ve described.  Our Case Study suggests
the ways in which progress towards realising these scenarios might be
made actionable.
...
introduced in our Analysis.  Dawes and Ostwald provided a
...
the course of work, and, moreover is not necessarily in touch with others elsewhere as the work develops.
...
as a source of processed data.  CLA and DPL can then be used to formulate a “formal picture”: as in our
...
/patterns as elements in the world/.  Some patterns repeat in space,
...
help people work with these methods.  As Turing said about the future,
...
Smith, David O’Toole, Rebecca Raper, and Steve Corneli.
...
of a randomised selection of a *Rule*, *Action*, *Place*, and *Issue*
...
which allows people to stay cool while socialising outside.  From a
