...
\hspace{.25\textwidth}\begin{minipage}{.75\textwidth}
  \emph{‘Sire,
    ...
    Saṁyutta Nikāya Connected Discourses on Causation 12.65. The City;
    quoted in \cite{batchelor2015after}
  \end{minipage}
#+END_EXPORT
* Abstract                                                        :noexport:
:PROPERTIES:
...
# scales.
We emphasise that design patterns, notwithstanding criticisms and
shortcomings, have been deployed in the real world, in a way that as
to tangle with all of these complexities.  Whereas science fiction can
...
face.  We need a concrete, testable, and adaptable approach that
integrates thinking with action and awareness, and that works across
...
methods within a distributed research seminar.
\begin{echo}
Details of these
...
In précis, the Analysis examined:
...
Our Case Study works at the small scale, and combines all three
methods in line with the [[PLACARD][PLACARD]] pattern introduced
above.  Specifically, in our research seminar, we used the PAR to
...
which could further broken down in subsequent work, using the Case Study
as a model.
...
- /the sky/ and /the world/ (Alexander of /A Pattern Language/ (/APL/), as seen from the dual vantage points of /the hearth/ and /the gateway/; cf. \textsc{The Fire} (pattern #181) and \textsc{Main gateway} (pattern #53), and cite:benvenuto1993hermes).
...
the CLA of design patterns in the Analysis could be developed further.  The
...
links between patterns and next steps, and additonally assisted by software tools.
...
Simply using a ‘copyleft’ license is not a panacea for all ills cite:Krowne_Puzio_2006, and
...
combined modelling with scenario planning cite:olabisi2010.
...
needed, or accepted.  Nevertheless, from a modelling perspective,
...
action components.  Despite the limitations of the present study when
it comes to scale, the evidence to support the use of the integrated
...
the first instance, and making their work more robust.
...
potentially important role to play here cite:wildschut2017a, and the
methods described may help to support widespread engagement.
